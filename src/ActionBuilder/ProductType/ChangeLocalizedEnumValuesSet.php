 ...
 */
class ChangeLocalizedEnumValuesSet extends ProductTypeActionBuilder
{
    /**
     * @var string The field name.
     */
    protected $complexFieldFilter = '^attributes\/(\w+)\/type\/elementType\/values$';
    ...
    public function createUpdateActions(
        ...
        // Only apply on _localized_ enums set
        if ($attribute['type']['elementType']['name'] !== 'lenum') {
            return [];
        }
        ...
        foreach ($oldData['attributes']  as $item) {
            if ($item['name'] === $attribute['name']) {
                foreach ($item['type']['elementType']['values'] as $value) {
